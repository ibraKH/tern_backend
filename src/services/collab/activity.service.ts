 ...
 * Resolves modelName → model_id internally so callers don't need the numeric ID.
 */
export async function logActivity(params: LogActivityParams): Promise<void> {
  try {
    const { modelName, userId, action, entityType, entityId, detail } = params;

    const modelResult = await pool.query<{ id: number }>(
    ...
    await pool.query(
      `INSERT INTO collab_activity (model_id, user_id, action, entity_type, entity_id, detail)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [modelResult.rows[0].id, userId, action, entityType ?? null, entityId ?? null, detail ?? null]
    );
  } catch (err) {
...
/**
 * Fetch the most recent activity entries for a model.
 * Returns an empty array if the model doesn't exist or has no activity.
 */
export async function getRecentActivity(
  ...
  const result = await pool.query<{
    ...
    `SELECT
     ...
     LIMIT $2`,
    [modelResult.rows[0].id, clampedLimit]
  );

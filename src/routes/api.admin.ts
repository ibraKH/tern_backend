...
adminRouter.delete("/users/:id/sessions", async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(`DELETE FROM sessions WHERE user_id = $1`, [targetId]);
...
adminRouter.delete("/users/:id", async (req: Request, res: Response) => {
  ...
  try {
    const lookup = await pool.query(`SELECT email FROM auth_users WHERE id = $1`, [targetId]);
